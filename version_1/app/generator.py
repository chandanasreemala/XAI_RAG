...
def generate_answer(
...
) -> Union[str, Tuple[str, Optional[float], Optional[list], Optional[list]]]:
    """
    Generate answer from prompt with optional confidence and token logprobs.
    
    Returns:
      - If return_sequence_confidence=False and return_token_logprobs=False: just str
      - If return_sequence_confidence=True or return_token_logprobs=True: 
        tuple of (str, seq_conf, token_texts, token_logprobs)
    """
    # Keep deterministic behavior (do_sample=False) for temperature=0.0
    try:
        output = generator(
            prompt,
            max_length=max_length,
        ...
        # Fallback if generator doesn't support these params
        output = generator(prompt)
        text = output[0].get("generated_text", "")

    if not return_sequence_confidence and not return_token_logprobs:
        return text
    ...
    if return_sequence_confidence or return_token_logprobs:
        confidence = _sequence_confidence_exp_mean_logprob(
            generator=generator,
            prompt=prompt,
            max_new_tokens=max_length,
    ...
    # Note: token-level logprobs would require more invasive changes to the pipeline
    # For now, we return None for token_texts and token_logprobs
    # A full implementation would need to access model.generate directly with output_scores=True
    
    return text, confidence, token_texts, token_logprobs
