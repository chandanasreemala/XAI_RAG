...
def compute_scores(
    ...
    comparator,
    generator,
    fusion_methods=("A", "B", "C"),
    fusion_lambda=0.5,
):
    """
    Implements:
        - Baseline RAG-Ex importance
        - Fusion A, B, C

    Fusion A:
        δ_i_norm * (λ R_i + (1-λ) ΔC_i_norm)

    Fusion B:
        (λ R_i + (1-λ) ΔC_i_norm)

    Fusion C:
        δ_i_norm * R_i * ΔC_i_norm
    """

    # --- 1. Response sensitivity (baseline RAG-Ex) ---
    delta = []
    perturbed_confidences = []

    for unit in units:
        answer_i, conf_i = generator(unit)
        sim = comparator(answer_i, baseline_answer)
        delta_i = 1.0 - sim
        delta.append(delta_i)
        perturbed_confidences.append(conf_i)

    delta_norm = normalize_max(delta)

    # --- 2. Confidence drop ---
    delta_c = [
        max(0.0, baseline_confidence - c) for c in perturbed_confidences
    ]
    delta_c_norm = minmax_norm(delta_c)

    # --- 3. Document relevance ---
    doc_scores_norm = minmax_norm(doc_scores)
    R_i = [doc_scores_norm[idx] for idx in unit_doc_indices]

    # --- 4. Fusion ---
    results = {}
    baseline_scores = delta_norm
    ...
    for method in fusion_methods:
        ...
        for i in range(len(units)):
            if method == "A":
                val = delta_norm[i] * (
                    fusion_lambda * R_i[i]
                    + (1 - fusion_lambda) * delta_c_norm[i]
                )
            elif method == "B":
                val = (
                    fusion_lambda * R_i[i]
            ...
            fusion_raw.append(val)

        results[f"fusion_{method}"] = normalize_max(fusion_raw)

    signals = {
        "delta_norm": delta_norm,
        "doc_relevance_norm": R_i,
        "confidence_drop_norm": delta_c_norm,
    }
